...
@Component({
  selector: "swc-landing-hotel-strategie-page",
  template: `
    <swc-hero-section-main
      class="relative block overflow-hidden"
      ctaLabel="Générez votre plan d’action hôtelier"
      skipNav
    >
      <ng-container title>
        <span class="font-bold">En 30 secondes,</span>
        identifiez tous les travaux possibles dans vos hôtels.
        <br />
        <span class="font-bold">Gratuitement.</span>
      </ng-container>

      <ng-container text>
        Optee transforme la donnée de vos établissements en projets concrets,
        prêts à être lancés : rénovation énergétique, mise aux normes,
        rafraîchissement des chambres ou optimisation technique.
      </ng-container>
    ...
    >
      ...
      <swc-select-professionals>
        <ng-container title>
          ...
          l’ensemble des opérations de rénovation pertinentes, selon les
          caractéristiques réelles de vos hôtels.
        </ng-container>
      </swc-select-professionals>

      <swc-registering-process altVersion />

      <oui-radar-register-now ctaLabel="Générer mon plan d’action hôtelier" />
      ...
      <oui-onboarding-button
        ...
        label="Générez votre plan d’action hôtelier"
      />
    </div>
  `,
  imports: [
...
export class LandingHotelStrategiePageComponent {}
